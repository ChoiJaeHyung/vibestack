...
CREATE TABLE public.technology_knowledge (
  ...
  technology_name_normalized TEXT NOT NULL,
  version TEXT,
  concepts JSONB NOT NULL DEFAULT '[]'::jsonb,
  source TEXT NOT NULL DEFAULT 'llm_generated' CHECK (source IN ('seed', 'llm_generated')),
  generation_status TEXT NOT NULL DEFAULT 'ready' CHECK (generation_status IN ('ready', 'generating', 'failed')),
  llm_provider TEXT,
  llm_model TEXT,
  ...
  UNIQUE(technology_name_normalized)
);

ALTER TABLE public.technology_knowledge ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read technology knowledge"
  ON public.technology_knowledge FOR SELECT
  TO authenticated
  USING (true);

-- service_role은 RLS를 bypass하므로 별도 INSERT/UPDATE 정책 불필요
-- (Supabase service_role key는 자동으로 RLS 우회)

-- ─── 시드 데이터: 5개 기술 ──────────────────────────────────────────

INSERT INTO public.technology_knowledge (technology_name, technology_name_normalized, version, concepts, source, generation_status, generated_at) VALUES
